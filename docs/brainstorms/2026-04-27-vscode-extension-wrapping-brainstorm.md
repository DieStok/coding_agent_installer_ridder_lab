...
Layered on top of the brief, the brainstorm-round added **automatic SLURM job allocation**: VSCode runs on the login node where there's no `SLURM_JOB_ID`, but our existing wrapper requires one. Rather than refusing every GUI click or asking users to launch VSCode from inside `srun --pty bash`, the wrapper auto-allocates a single per-Cursor-session SLURM job and routes every spawn into it via `srun --jobid=<cached>`. First-spawn cost (5–15 s) is hidden inside extension auto-activation (which fires on workspace-open before the user clicks anything); every spawn after that is ~200 ms–2 s. Existing `SLURM_JOB_ID` (e.g., user already ran `srun --pty bash`) is reused.
...
**Decided 2026-04-27.** When `SLURM_JOB_ID` is unset and no usable cached jobid exists, the shim runs `salloc --account=compgen --time=08:00:00 --mem=10G --cpus-per-task=2 --no-shell --job-name=cod-ag-vscode-${USER}-${$}` to allocate a single per-session job, caches the resulting JOB_ID at `~/.coding-agents/vscode-session.jobid` (under `flock`), and runs every subsequent spawn via `srun --jobid=$JOB_ID`. If `salloc` fails (account out, partition full, network blip), the shim refuses with a clear error in the VSCode notification — and the next spawn 30+ seconds later attempts `salloc` exactly once more before refusing again. Rejected: silently degrading to unwrapped (would defeat the whole point), or always wrapping without SLURM (loses accounting).
...
**Decided 2026-04-27.** The lab's actual setup is VSCode (`TERM_PROGRAM=vscode`, version 1.104.x). Cursor uses the same extension API and same remote-server `data/User/settings.json` layout, so the implementation works for both. Where the two diverge (Cursor desktop SQLite vs VSCode remote JSON; the launcher being `code-wrapped` for VSCode and `cursor-wrapped` for Cursor) the docs note both. The deep-research brief was rewritten to reflect this distinction explicitly.
